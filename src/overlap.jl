...
located at `r` relative to `ϕ2`, given in Angstrom.
...
"""
function overlap(skt::SlaterKosterTable, r, ϕ1, ϕ2)
    ...
    return overlap(table, r, last(ϕ1), last(ϕ2))
end

"""
    overlap(skt::SlaterKosterTable, ϕ)

Return the diagonal overlap matrix element corresponding to `ϕ`.
...
"""
overlap(skt::SlaterKosterTable, ϕ) = overlap(skt.data[first(ϕ), first(ϕ)], last(ϕ))

overlap(skt::HomoNuclearTable, orbital_symbol::Symbol) = 1

function overlap(skt::PrimitiveTable, r, orbital_symbol1::Symbol, orbital_symbol2::Symbol)
    ...
    d = r ./ norm_r
    return overlap(table, d, Val(orbital_symbol1), Val(orbital_symbol2))[]
end

function overlap(df::DataFrame, d, ::Val{:s}, ::Val{:s})
    return df.Sss0
end

for (i, x) in [(1, :(:x)), (2, :(:y)), (3, :(:z))]
    @eval @inline function overlap(df::DataFrame, d, ::Val{:s}, ::Val{Symbol("p", $x)})
        return d[$i]*df.Ssp0
    end

    for (j, y) in [(1, :(:x)), (2, :(:y)), (3, :(:z))]
        if i == j
            @eval @inline function overlap(df::DataFrame, d, ::Val{Symbol("p", $x)}, ::Val{Symbol("p", $x)})
                return d[$i]^2*df.Spp0 + (1 - d[$i]^2)*df.Spp1
            end
        else
            @eval @inline function overlap(df::DataFrame, d, ::Val{Symbol("p", $x)}, ::Val{Symbol("p", $y)})
                return d[$i]*d[$j]*(df.Spp0 - df.Spp1)
            end
        end
    end
end
